...
/**
 * 마지막으로 통보한 가격(기준선)과 새 가격을 비교해 알릴 변동을 판정한다.
 * 워커의 알림 로직 전부가 여기 있다.
 *
 * @param previous 마지막으로 실제 통보된 가격. [TrackedRoute.notifiedPrice].
...
class DetectPriceChangesUseCase @Inject constructor() {
    ...
    operator fun invoke(
        ...
        return PriceChange(
            ...
            previous = previous,
            current = current,
            direction = if (current < previous) Direction.DOWN else Direction.UP,
