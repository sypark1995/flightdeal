...
**선택지에 도시 코드를 넣지 않는다.** 응답은 모든 행에 `origin: "SEL"`(도시)과
`origin_airport: "ICN"`(공항)을 함께 주고 매퍼는 공항 코드를 우선한다. 인천 고정일
때는 조회한 것과 저장되는 것이 우연히 같았지만, `SEL`을 고를 수 있게 하면 인천발과
김포발이 섞여 와 행마다 다른 공항이 저장된다. 같은 노선이 추적 항목 두 줄로 갈리고,
`GMP→TYO`는 `ICN→TYO`보다 표본이 훨씬 얇아 할인 기준선도 카드마다 달라진다.
**공항 코드만 쓰면 조회한 것과 저장되는 것이 항상 같다.**

목록에 없는 IATA가 저장돼 있으면(선택지가 줄어든 버전으로 올라가면 그렇게 된다)
예외를 던지지 않고 기본값으로 돌아간다. 그 자리에서 던지면 앱이 열리지 않는다.
...
- 출발지를 제주로 바꿨을 때 라벨은 "제주 출발"로 바뀌는데 **목록은 이전 출발지의
  가격을 그대로** 보여줬다. "조회에 실패해도 보던 목록을 지우지 않는다"는 규칙을
  조건이 바뀐 조회에까지 적용한 탓이다 — **오래된 목록을 붙잡는 것은 같은 조건을
  다시 조회할 때만 옳다. 조건이 바뀌면 이전 결과는 새 질문에 대한 답이 아니다.**
  이 규칙을 세우고 나니 어긋남을 사후에 고치던 `revertTripType`이 통째로 필요
  없어졌다 — **고쳐야 할 상태를 없애는 쪽이 고치는 코드를 두는 쪽보다 낫다.**
